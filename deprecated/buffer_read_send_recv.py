...
def main():
    ...
    if rank == 0:
        ...
        with open(twitter_data, 'rb') as f:
            ...
            for chunk in ijson.items(parser, 'item'):
                ...
                if len(tweet_buffer) == BUFFER_SIZE:
                    if current_process == 0:
                        ...
                        comm.send(tweet_buffer, dest=current_process)
                    tweet_buffer = []
            
                    # Update the process number to send next tweet
            ...
            # Send stop signals to other processes
            for i in range(1, size):
                comm.send(None, dest=i)

    else:
        sub_city_tweet_counts = {}
        sub_author_tweet_counts = {}
        sub_author_city_counts = {}
    
        while True:
            tweet_buffer = comm.recv(source=0)
            if tweet_buffer is None:
                break
            ...
            # Update the sub results
            update_sub_results(sub_city_tweet_counts, sub_author_tweet_counts, sub_author_city_counts, buffer_city_tweet_counts, buffer_author_tweet_counts, buffer_author_city_counts)

    
    
    # # Process the data
    # sub_city_tweet_counts, sub_author_tweet_counts, sub_author_city_counts = data_process(sal_data, tweet_group)
